...
if(isset($_GET['name']) && isset($_GET['factory'])){
    $sth = $dbh->prepare('SELECT commodity_name, cost, sell_price, factory_name from commodity Left Join factory on factory.factory_tax_id = commodity.factory_tax_id WHERE commodity_name=? AND factory_name=? AND user_ID=?');
...
if(isset($_POST['cost']) && isset($_POST['price']) && isset($_POST['name']) && isset($_POST['factory'])){
    $sth = $dbh->prepare('UPDATE commodity Left Join factory ON factory.factory_tax_id = commodity.factory_tax_id SET cost=?,sell_price=? WHERE commodity_name=? AND factory_name=? AND user_ID=?');
    $sth->execute(array(
        $_POST['cost'],
        $_POST['price'],
        $_POST['name'],
